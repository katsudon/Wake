...
char * get_ascii (ROOM_INDEX_DATA * pRoom, bool fWall)
{
	...
	if (!fWall){
		switch (pRoom->sector_type){
			...
			case SECT_VOLCANO:		return "{R.";
			case SECT_RAINFOREST:	return "{g^";
			case SECT_SWAMP:		return "{c~";
			case SECT_RIVER:		return "{B~";
			case SECT_TUNNEL:		return "{d.";
			case SECT_FORESTCITY:	return "{g.";
			case SECT_MOUNTAINCITY:	return "{y.";
			case SECT_DESERTCITY:	return "{y.";
			case SECT_GRAVEYARD:	return "{d.";
			case SECT_ROAD:			return "{d.";
			case SECT_DIRTROAD:		return "{y.";
			case SECT_CITYROAD:		return "{w.";
			case SECT_DIRTPATH:		return "{y.";
			case SECT_GOODTEMPLE:	return "{x.";
			case SECT_NEUTRALTEMPLE:return "{w.";
			case SECT_EVILTEMPLE:	return "{d.";
			case SECT_CLIFFSIDE:	return "{y.";
			case SECT_ICETUNNEL:	return "{C.";
			case SECT_BOARDWALK:	return "{Y=";
			case SECT_BEACH:		return "{Y~";
			case SECT_SEWER:		return "{d.";
			default:			return "{RX";
		}
	}
	else
...
char * add_map_char (CHAR_DATA *ch, int x, int y, int x_pos, int y_pos)
{
	...
	else if (x_pos == 2 && y_pos == 0) //Upper Right
	{
		...
		//Pretty up fields and large halls by removing unsightly corners.
					...
					&& !IS_SET (pExit->exit_info, EX_ISDOOR)
					&& !(IS_SET (pExit->exit_info, EX_SDOOR) && IS_SET(pExit->exit_info,EX_CLOSED))
					&& !IS_SET (pExit->exit_info, EX_HIDDEN)
...
void create_map (CHAR_DATA * ch)
{
	...
	while (g_list[0][0] != 0)
	{
		...
		map3[g_list[0][1]][g_list[0][2]] = g_list[0][0];
		for (door = 0; door < MAX_MAP_DIR; door++)
		{
					...
					&&   !IS_SET (pexit->exit_info, EX_HIDDEN))
			{
				...
				if (fSearch || map3[x+rev_dirs[door][0]][y+rev_dirs[door][1]] != 0)
					continue;
...
void display_map (CHAR_DATA * ch)
{
	...
	bzero (map3, sizeof(map3));
	create_map(ch);
	desc2 = str_dup(ch->in_room->description);
	strcpy(desc,format_map_desc(desc2));
    ...
    for (y = min_y; y < max_y; ++y){
		for (y_pos = 0; y_pos < 3; y_pos++)
		{
			...
			{
				room_wall = get_ascii (get_room_index (map3[x][y]), true);
				room_floor = get_ascii (get_room_index (map3[x][y]), false);
			...
			if (y == min_y && y_pos == 0)
			{
				sprintf (buf, "{D| {G %s",  ch->in_room->name);
				strcat (map_buf, buf);
			}
			else if (y != max_y && y_pos < 3)
			{
				...
				strcat (map_buf, "   ");
				strcat(map_buf,sect_table[ch->in_room->sector_type].color);
				strcat (map_buf, line);
				strcat (map_buf, "{x");
			}
			strcat (map_buf, "\n\r");
...
void do_newmap(CHAR_DATA *ch,char *argument){
	display_map(ch);
}
...
char *format_map_desc (char *oldstring)
{
    ...
    for (;;)
    {
        ...
        {
			bug ("format_map_desc: No spaces", 0);
            *(rdesc + 45) = 0;
            strcat (xbuf, rdesc);
            strcat (xbuf, "-\n\r");
            rdesc += 46;
