...
*/ 
#define BLACK			"\x1B[1;37m\x1B[30m"
#define WHITE			"\x1B[0;37m"
#define CYAN			"\x1B[0;36m"
#define NO_COLOR		"\x1B[1;37m"

#define L_SUP (MAX_LEVEL - 1) /* if not already defined */


BOARD_DATA boards[MAX_BOARD]={
// short,        long							readlvl				wrtelvl			dfltreciever	defaction		defexpire	fstNote unused?
...
{ "Changes", 	"Changes to the game",			 0,					MAX_LEVEL-2,	"all",			DEF_INCLUDE,	100,		NULL,	false },
{ "Personal", 	"Personal messages",			 0,					2,				"all",			DEF_EXCLUDE,	28,			NULL,	false },
{ "Immortals",	"Immortal business",		     LEVEL_IMMORTAL,	LEVEL_IMMORTAL,	"imm",			DEF_INCLUDE,	500,		NULL,	false },
{ "Ideas",	  	"Suggestion for improvement",	 0,					2,				"all",			DEF_NORMAL,		60,			NULL,	false },
{ "Bugs",	  	"Typos, bugs, errors",		     0,					2,				"imm",			DEF_NORMAL,		60,			NULL,	false },
{ "Punishment", "Punishments and reasons",		 0,					LEVEL_IMMORTAL,	"all",			DEF_EXCLUDE,	100,		NULL,	false }
};

/* The prompt that the character is given after finishing a note with ~ or END */
const char * szFinishPrompt = NO_COLOR "({cC{x)ontinue, ({cV{x)iew, ({cP{x)ost or ({cF{x)orget it?";
...
static void show_note_to_char (CHAR_DATA *ch, NOTE_DATA *note, int num)
{
    ...
    output = new_buf();
	/* Ugly colors ? */	
	printf_to_char(ch,"[{c%4d{x] {d%s{x: {c%s{x\n\r{dDate{x:  %s\n\r{dTo{x:    %s\n\r{c==========================================================================={x\n\r",
...
static void do_nwrite (CHAR_DATA *ch, char *argument){
	...
	if (!ch->pcdata->in_progress->text) /* Are we continuing an old note or not? */
	{
		...
		{
		case DEF_NORMAL:
			printf_to_char(ch,"If you press Return, default recipient \"{w%s{x\" will be chosen.\n\r",ch->pcdata->board->names);
			break;
		case DEF_INCLUDE:
			printf_to_char(ch,"The recipient list MUST include \"{w%s{x\". If not, it will be added automatically.\n\r",ch->pcdata->board->names);
			break;
		case DEF_EXCLUDE:
			printf_to_char(ch,"The recipient of this note must NOT include: \"{w%s{x\".",ch->pcdata->board->names);
			break;
		}
		send_to_char("\n\r{cTo{x:      ",ch);
...
static void do_nread (CHAR_DATA *ch, char *argument)
{
	...
	else /* just next one */
	{
		...
		if (next_board (ch))
			printf_to_char(ch,"Changed to next board, {w%s{x.\n\r", ch->pcdata->board->short_name);
		else
			printf_to_char(ch,"There are no more boards.\n\r");
	}
}
...
static void do_nremove (CHAR_DATA *ch, char *argument)
{
	...
	if (str_cmp(ch->name,p->sender) && (get_trust(ch) < MAX_LEVEL-2))
	{
...
void do_board(CHAR_DATA *ch,char *argument){
	...
	if (!argument[0]){//show boards
		...
		count = 1;
		ch->send("{d ______________________________________________________\n\r");
		ch->send("/                                                      \\{x\n\r");
		ch->send("|Num         Name Unread Description                   {d|\n\r");
		ch->send("|{w=== ============ ====== =============================={d|\n\r");
		for (i = 0;i < MAX_BOARD;i++){
			unread = unread_notes(ch,&boards[i]);/* how many unread notes? */
			if (unread != BOARD_NOACCESS){
				printf_to_char(ch,"{d|{c%2d{x> {g%12s{x [%s%4d{x] {d%-30s|{x\n\r",count, boards[i].short_name, unread ? "{C" : "{c",unread,boards[i].long_name);
				count++;
			} /* if has access */
		} /* for each board */
		ch->send("{d\\______________________________________________________/\n\r");

		printf_to_char(ch,"\n\rYou current board is {c%s{x.\n\r",ch->pcdata->board->short_name);
	...
	if (ch->pcdata->in_progress){
		ch->send("Please finish your interrupted note first.\n\r");
		return;
	}
	...
	switch (ch->pcdata->board->force_type)
	{
		case DEF_NORMAL: /* default field */
			if (!buf[0]) /* empty string? */
			{
				...
				printf_to_char(ch,"Assumed default recipient: {w%s{x\n\r", ch->pcdata->board->names);
			}
			else
...
void handle_con_note_text (DESCRIPTOR_DATA *d, char * argument)
{
	...
	if ((strlen_color(letter) + strlen (buf)) > MAX_NOTE_TEXT)
...
void handle_con_note_finish (DESCRIPTOR_DATA *d, char * argument){
	...
	switch (tolower(argument[0])){
		...
		case 'v': /* view note so far */
			if (ch->pcdata->in_progress->text)
			{
				send_to_char("{wText of your note so far:{x\n\r",ch);
				send_to_char(ch->pcdata->in_progress->text,ch);
			}
			else
		...
		case 'p': /* post note */
			...
			/* remove AFK status */
			ch->pcdata->in_progress = NULL;
			act("$n finishes $s note.{x",ch,NULL,NULL,TO_ROOM);
			sprintf(buf,"New note posted on the %s board.{x\n\r",ch->pcdata->board->short_name);
			global_message(ch->pcdata->board->read_level,MAX_LEVEL,buf,CM_NONOTES);
			break;
		case 'f':
