...
extern int map2[MAX_MAP][MAX_MAP];
extern MAP_PARAM *mp;
...
void init_mp (CHAR_DATA *ch)
{
	...
	if (mp == NULL)
	{
		wiznet ("New map_param created", NULL, NULL, WZ_FLAGS, 0, 0);
...
MAP_VISIT *new_map_visit (void)
{
	...
	{
		wiznet ("New map_visit created", NULL, NULL, WZ_FLAGS, 0, 0);
...
MAP_QD *new_map_qd (void)
{
	...
	if (map_qd_free == NULL)
	{
		wiznet ("New map_qd created", NULL, NULL, WZ_FLAGS, 0, 0);
			  ...
			  int ly)
{
	...
	if (mp->border)
	{
		printf_to_char (ch, "%s+", mpcolour("{Y"));
	...
	for (y = y_from; y <= y_to; y++){
		for (x = x_from; x <= x_to; x++){
			if (map2[x][y] != -1){// Is there something here?
				if (map2[x][y] < -1) // ? < -1 means a link type.  Lets buffer it
					sprintf (buf, "%s%s%s", buf, get_linkc(map2[x][y]), get_link(map2[x][y]));
				else if (lx == x && ly == y){// This is where we started.  Make sure to mark it with a special character
					if (mp->col == true)
					...
					sprintf (buf, "%s%s%s", buf, get_mroomc(ch, map2[x][y]), get_mroom(ch, map2[x][y]));
				}
			}
		...
		{
			if (mp->col == true)
				printf_to_char (ch, "{Y|%s%s|", buf, mpcolour("{w{Y"));
			else
				printf_to_char (ch, "{Y|%s%s|", buf, mpcolour("{Y"));
	...
	if (mp->border)
	{
		...
		for (x = x_from; x <= x_to; x++)
			send_to_char ("-", ch);
		printf_to_char (ch, "+%s", mpcolour("{w"));
				...
				case DIR_NORTH: // y - 1
					if (!tight) // Do we want our room links?
					{
						...
						if (y - 2 < 0) // Will we oversteap our bounds if we put try to put in this link?
							...
							map2[x][y-1] = put_link (room, map2[x][y-2], dir);
					}
					else
						num = 1;
					...
					map2[x][y-num] = exit->u1.to_room->vnum;
					qd = new_map_qd();
					qd->x = x;
					qd->y = y - num;
				...
				case DIR_EAST: // x + 1
					if (!tight) // See all comments above.
					{
						...
						if (x + 2 > MAX_MAP - 1)
							...
							map2[x+1][y] = put_link (room, map2[x+2][y],  dir);
					}
					else
						num = 1;
					...
					if (x + num > MAX_MAP - 1
						...
						|| has_visited(exit->u1.to_room->vnum))
						break;
					map2[x+num][y] = exit->u1.to_room->vnum;
					qd = new_map_qd();
					qd->x = x + num;
					qd->y = y;
					qd->room = exit->u1.to_room->vnum;
