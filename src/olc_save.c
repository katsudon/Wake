 ...
 * -- Hugin
 */
char *fwrite_flag(l_int flags, char buf[] )
{
    char offset;
    char *cp;
...
void save_mobile( FILE *fp, MOB_INDEX_DATA *pMobIndex )
{
    ...
    fprintf(fp,"%d %d\n",		pMobIndex->pclass, pMobIndex->trainer );
    fprintf(fp,"%s~\n",		pMobIndex->icpmsg );
    fprintf(fp,"%s~\n",		pMobIndex->ocpmsg );
    fprintf(fp,"%s~\n",		pMobIndex->cmsg );
    ...
    fprintf(fp,"%dd%d+%d ",	pMobIndex->hit[DICE_NUMBER], pMobIndex->hit[DICE_TYPE], pMobIndex->hit[DICE_BONUS] );
    fprintf(fp,"%dd%d+%d ",	pMobIndex->mana[DICE_NUMBER], pMobIndex->mana[DICE_TYPE], pMobIndex->mana[DICE_BONUS] );
    fprintf(fp,"%dd%d+%d ",	pMobIndex->damage[DICE_NUMBER], pMobIndex->damage[DICE_TYPE], pMobIndex->damage[DICE_BONUS] );
    fprintf(fp,"%s\n",		attack_table[pMobIndex->dam_type].name );
    fprintf(fp,"%d %d %d %d\n",pMobIndex->ac[AC_PIERCE] / 10, pMobIndex->ac[AC_BASH]   / 10, pMobIndex->ac[AC_SLASH]  / 10, pMobIndex->ac[AC_EXOTIC] / 10 );
    fprintf(fp,"%s %s %s %d %d\n",position_table[pMobIndex->start_pos].short_name,position_table[pMobIndex->default_pos].short_name,sex_table[pMobIndex->sex].name,pMobIndex->wealth,pMobIndex->guild);
    fprintf(fp,"%s ",		size_table[pMobIndex->size].name );
    fprintf(fp,"%s\n",		!IS_NULLSTR(pMobIndex->material) ? pMobIndex->material : "unknown" );
    fprintf(fp,"%s\n",		save_bits(pMobIndex->act,MAX_ACT));
    fprintf(fp,"%s\n",		save_bits(pMobIndex->affected_by,MAX_AFF));
    fprintf(fp,"%s\n",		save_bits(pMobIndex->off_bits,MAX_OFF));
    fprintf(fp,"%s\n",		save_bits(pMobIndex->def_bits,MAX_DEF));
    fprintf(fp,"%s\n",		save_ints(pMobIndex->res,MAX_RES));
    fprintf(fp,"%s\n",		save_bits(pMobIndex->form,MAX_FRM));
    fprintf(fp,"%s\n",		save_bits(pMobIndex->parts,MAX_PRT));
		fprintf(fp," %d",pMobIndex->perm_stat[0]);
	for (i=1;i<MAX_STATS;i++)
		fprintf(fp," %d",pMobIndex->perm_stat[i]);
		fprintf(fp,"\n");

    for (pMprog = pMobIndex->mprogs; pMprog; pMprog = pMprog->next)
        fprintf(fp, "M %s %d %s~\n",prog_type_to_name(pMprog->trig_type), pMprog->vnum,pMprog->trig_phrase);
...
void save_object( FILE *fp, OBJ_INDEX_DATA *pObjIndex )
{
    ...
    fprintf(fp,"%s\n",		fwrite_flag(pObjIndex->exclude, buf));
	fprintf(fp,"%s\n",		save_bits(pObjIndex->wflags,MAX_WPN));
/*
 ...
 */
	...
	fprintf(fp,"%d ",pObjIndex->droprate);
	...
	for (pOprog = pObjIndex->oprogs; pOprog; pOprog = pOprog->next){
		fprintf(fp, "O %s %d %s~\n",
		prog_type_to_name(pOprog->trig_type), pOprog->vnum, pOprog->trig_phrase);
	}
	return;
}
...
void save_rooms( FILE *fp, AREA_DATA *pArea )
{
    ...
    for( iHash = 0; iHash < MAX_KEY_HASH; iHash++ ){
                ...
                for (door = 0; door < MAX_DIR; door++){
                    if ((pExit = pRoomIndex->exit[door]) && pExit->u1.to_room){
						...
						||   IS_SET(pExit->rs_flags, EX_NOLOCK) )
							...
							fprintf( fp, "%s~\n",		pExit->keyword );
							fprintf( fp, "%d %d %d\n",	0,pExit->key,pExit->u1.to_room->vnum);
		...
		if (pRoomIndex->arenavn > 0)
			fprintf(fp,"A %d\n",pRoomIndex->arenavn);
		if (pRoomIndex->arenavn > 0)
			fprintf(fp,"V %d\n",pRoomIndex->arenaviewvn);
