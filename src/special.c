...
bool spec_patrolman(CHAR_DATA *ch)
{
	...
	if (!IS_AWAKE(ch) || ch->isaff(AF_CALM) || ch->in_room == NULL || ch->isaff(AF_CHARM) || ch->fighting != NULL)
		return false;
	...
	switch (number_range(0,6))
	{
		default:
			...
			break;
		case 0:
			message = "All roit! All roit! break it up!";
			do_function(ch, &do_yell, message );
			break;
		case 1:
			message = "Society's to blame, but what's a bloke to do?";
			do_function(ch, &do_say, message );
			break;
		case 2:
			message = "Bloody kids will be the death of us all.";
			do_function(ch, &do_say, message );
			break;
		case 3:
			message = "Stop that! Stop that!";
			do_function(ch, &do_shout, message );
			break;
		case 4:
			act("$n pulls out his billy and goes to work.",ch,NULL,NULL,TO_ROOM);
			break;
		case 5:
			act("$n sighs in resignation and proceeds to break up the fight.",ch,NULL,NULL,TO_ROOM);
			break;
		case 6:
			message = "Settle down, you hooligans!";
			do_function(ch, &do_say, message );
			break;
	}
...
bool dragon( CHAR_DATA *ch, char *spell_name )
{
    ...
    if ( ( sn = skill_lookup( spell_name ) ) < 0 )
		return false;
    (*skill_table[sn].spell_fun) ( sn, ch->level, ch, victim, TARGET_CHAR,1);//nash slvl
    return true;
}
...
bool spec_breath_gas( CHAR_DATA *ch )
{
    ...
    if ( ( sn = skill_lookup( "gas breath" ) ) < 0 )
		return false;
    (*skill_table[sn].spell_fun) ( sn, ch->level, ch, NULL,TARGET_CHAR,1);//nash slvl
    return true;
}
...
bool spec_cast_adept( CHAR_DATA *ch )
{
    ...
    switch ( number_bits( 4 ) )
    {
		case 0:
			...
			spell_armor( skill_lookup( "armor" ), ch->level,ch,victim,TARGET_CHAR,1);//nash slvl
			return true;
		case 1:
			act( "$n utters the word 'fido'.", ch, NULL, NULL, TO_ROOM );
			spell_bless( skill_lookup( "bless" ), ch->level,ch,victim,TARGET_CHAR,1);//nash slvl
			return true;
		case 2:
			act("$n utters the words 'judicandus noselacri'.",ch,NULL,NULL,TO_ROOM);
			spell_cure_blindness( skill_lookup( "cure blindness" ), ch->level, ch, victim,TARGET_CHAR,1);//nash slvl
			return true;
		case 3:
			act("$n utters the words 'judicandus dies'.", ch,NULL, NULL, TO_ROOM );
			//spell_cure_light( skill_lookup( "cure light" ), ch->level, ch, victim,TARGET_CHAR,1); nash slvl
			return true;
		case 4:
			act( "$n utters the words 'judicandus sausabru'.",ch,NULL,NULL,TO_ROOM);
			spell_cure_poison( skill_lookup( "cure poison" ), ch->level, ch, victim,TARGET_CHAR,1);//nash slvl
			return true;
		case 5:
			act("$n utters the word 'candusima'.", ch, NULL, NULL, TO_ROOM );
			spell_refresh( skill_lookup("refresh"),ch->level,ch,victim,TARGET_CHAR,1);//nash slvl
			return true;
		case 6:
			act("$n utters the words 'judicandus eugzagz'.",ch,NULL,NULL,TO_ROOM);
			spell_cure_disease(skill_lookup("cure disease"),ch->level,ch,victim,TARGET_CHAR,1);//nash slvl
			return true;
    }
...
bool spec_executioner( CHAR_DATA *ch )
{
    ...
    for ( victim = ch->in_room->people; victim != NULL; victim = v_next )
    {
		...
		if ( !IS_NPC(victim) && victim->isplr(PL_KILLER) && can_see(ch,victim))
		{
			crime = "KILLER";
			break;
		}

		if ( !IS_NPC(victim) && victim->isplr(PL_THIEF) && can_see(ch,victim))
		{
			crime = "THIEF";
			break;
		}
...
bool spec_guard( CHAR_DATA *ch )
{
	...
	for ( victim = ch->in_room->people; victim != NULL; victim = v_next )
	{
		...
		if ( !IS_NPC(victim) && victim->isact(PL_KILLER) && can_see(ch,victim))
		{
			crime = "KILLER";
			break;
		}

		if ( !IS_NPC(victim) && victim->isact(PL_THIEF) && can_see(ch,victim))
		{
			crime = "THIEF";
			break;
		}
	...
	if ( ech != NULL )
	{
		do_function(ch, &do_yell, "PROTECT THE INNOCENT!!  BANZAI!!");
		multi_hit( ch, ech, TYPE_UNDEFINED,false);
		return true;
	}
...
bool spec_mayor( CHAR_DATA *ch )
{
	...
	switch ( path[pos] )
	{
		...
		case '2':
		case '3':
			move_char( ch, path[pos] - '0', false, true );
			break;
		case 'W':
		...
		case 'S':
			...
			break;
		case 'a':
			do_function(ch, &do_say, "Hello Honey!");
			break;
		case 'b':
			do_function(ch, &do_say, "What a view!  I must do something about that dump!");
			break;
		case 'c':
			do_function(ch, &do_say, "Vandals!  Youngsters have no respect for anything!");
			break;
		case 'd':
			do_function(ch, &do_say, "Good day, citizens!");
			break;

		case 'e':
			do_function(ch, &do_say, "I hereby declare the city of Midgaard open!'");
			break;
		case 'E':
			do_function(ch, &do_say, "I hereby declare the city of Midgaard closed!");
			break;
		case 'O':
...
bool spec_poison( CHAR_DATA *ch )
{
    ...
    spell_poison( gsn_poison, ch->level, ch, victim,TARGET_CHAR,1);//nash slvl
    return true;
}
