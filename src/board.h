...
void finish_note (BOARD_DATA*,NOTE_DATA*); /* attach a note to a board */
void free_note   (NOTE_DATA*); /* deallocate memory used by a note */
void load_boards (void); /* load all boards */
int board_lookup (const char*); /* Find a board with that name */
bool is_note_to (CHAR_DATA*,NOTE_DATA*); /* is tha note to ch? */
void personal_message (const char*,const char*,const char*,const int,const char*);
void make_note (const char*,const char*,const char*,const char*,const int,const char*);
void save_notes ();

/* for nanny */
void handle_con_note_to 		(DESCRIPTOR_DATA*,char*);
void handle_con_note_subject 	(DESCRIPTOR_DATA*,char*);
void handle_con_note_expire 	(DESCRIPTOR_DATA*,char*);
void handle_con_note_text 		(DESCRIPTOR_DATA*,char*);
void handle_con_note_finish 	(DESCRIPTOR_DATA*,char*);
