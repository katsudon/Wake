...
const char * fn_keyword[]={
    ...
    "sameguild",		/* if sameguild $n	- guild check */
    "\n"			/* Table terminator */
};
 ...
 */
CHAR_DATA *get_random_pc(CHAR_DATA *mob,OBJ_DATA *obj,ROOM_INDEX_DATA *room){
	CHAR_DATA *vch, *victim = NULL;
	int now = 0, highest = 0;

	if ( (mob && obj) || (mob && room) || (obj && room) ){
		bug( "get_random_pc received multiple prog types",0);
		return NULL;
	}
...
CHAR_DATA *get_random_char(CHAR_DATA *mob,bool tokill){
	CHAR_DATA *vch, *victim = NULL;
	int now = 0, highest = 0;

	if (!mob){
		bug("get_random_char.2 received no mob",0);
		return NULL;
	}
	...
	for(vch = mob->in_room->people;vch;vch = vch->next_in_room){
		...
		&& can_see(mob,vch)
		&& (now = number_percent()) > highest
		&& (!tokill || (canFight(mob,vch,false) && !is_same_group(mob,vch)))){
			victim = vch;
			highest = now;
		}
	}
...
CHAR_DATA *get_random_char(CHAR_DATA *mob,OBJ_DATA *obj,ROOM_INDEX_DATA *room){
	...
	if (mob)
		...
		bug("get_random_char: received no input",0);
		return NULL;
	}
...
int count_people_room(CHAR_DATA *mob,OBJ_DATA *obj,ROOM_INDEX_DATA *room,int iFlag){
	...
	log_f("count_people_room: %d",count);
	return (count);
}
	...
	const void *arg1, const void *arg2, CHAR_DATA *rch )
{
	...
	if ( rval >= 0 ){
		...
		lval = rval;
		rval = atoi( buf );
		log_f("CMD_EVAL: c%s m%s l%d o%d r%d oo%s",ch->name, mob->name,lval,oper,rval,fn_evals[oper]);
	...
	switch( code )
	{
		...
		case 't':
			lval_char = vch; break;
		case 'r':
			lval_char = rch == NULL ? get_random_pc( mob, NULL, NULL ) : rch ; break;
		case 'o':
			lval_obj = obj1; break;
		case 'p':
			lval_obj = obj2; break;
	...
	switch( check )
	{
		...
		case CHK_SAMEGUILD:
			return( lval_char != NULL && ( ch->guild ==  mob->guild || ch->guild == 0) );
		case CHK_ISNPC:
			return( lval_char != NULL && IS_NPC( lval_char ) );
		case CHK_ISGOOD:
		...
		case CHK_ISCHARM: /* A relic from MERC 2.2 MOBprograms */
			return( lval_char != NULL && lval_char->isaff(AF_CHARM ) );
		case CHK_ISFOLLOW:
	...
	switch( check ){
		case CHK_AFFECTED:
			...
			&&  lval_char->isaff(flag_lookup(buf,affect_flags)) );
		case CHK_ACT:
			return( lval_char != NULL 
			&&  lval_char->isact(flag_lookup(buf,act_flags)) );
		case CHK_IMM:
			return(lval_char != NULL && lval_char->res[flag_lookup(buf,res_flags)] == 0);
		case CHK_OFF:
			return( lval_char != NULL 
			&&  lval_char->isoff(flag_lookup(buf,off_flags)));
		case CHK_CARRIES:
			if ( is_number( buf ) )
		...
		case CHK_NAME:
			switch( code ){
				...
				case 'r':
		case 'q':
			return(lval_char && is_name( buf, lval_char->name ) );
		case 'o':
		case 'p':
			return(lval_obj && is_name( buf, lval_obj->name ) );
		}
		case CHK_CLASS:
			return(lval_char && lval_char->pclass == class_lookup(buf));
		case CHK_POS:
	...
	if ( (oper = keyword_lookup( fn_evals, buf )) < 0 )
	{
		bugf("Cmd_eval_mob: prog %d syntax error(5): '%s' '%s'",vnum,original,buf);
		return false;
	}
	one_argument( line, buf );
	rval = atoi( buf );
	...
	switch( check )
	{
		...
		case CHK_GRPSIZE:
			if (lval_char)
				lval = count_people_room( lval_char, NULL, NULL, 4 ); break;
		default:
			return false;
	}
	if(ch && mob)
		log_f("CMD_EVAL: c%s m%s l%d o%d r%d",ch->name, mob->name,lval,oper,rval);//nash please
	return( num_eval( lval, oper, rval ) );
}
	...
	const void *arg1, const void *arg2, CHAR_DATA *rch )
{
    ...
    if ( buf[0] != '$' || buf[1] == '\0' )
    {
		bugf("Cmd_eval_obj: prog %d syntax error(3) '%s'",vnum,original);
        return false;
    }
    else
        code = buf[1];
    switch( code )
    {
        ...
        case 't':
            lval_char = vch; break;
        case 'r':
            lval_char = rch == NULL ? get_random_pc( NULL, obj, NULL ) : rch ; break;
        case 'o':
            lval_obj = obj1; break;
        case 'p':
            lval_obj = obj2; break;
	...
	default:
	    bugf("Cmd_eval_obj: prog %d syntax error(4) '%s'",vnum,original);
	    return false;
    }
    /*
     * From now on, we need an actor, so if none was found, bail out
	...
	*/
	switch( check )
	{
     ...
     {
	case CHK_AFFECTED:
	    return( lval_char != NULL && lval_char->hasaffs(buf) );
	case CHK_ACT:
	    return( lval_char != NULL && lval_char->hasacts(buf) );
	case CHK_IMM:
	    return false;//( lval_char != NULL && lval_char->hasimms(buf) ); NASHNEEDSTOFIXTHIS
	case CHK_OFF:
	    return( lval_char != NULL && lval_char->hasoffs(buf) );
	case CHK_CARRIES:
	    if ( is_number( buf ) )
		return( lval_char != NULL && has_item( lval_char, atoi(buf), -1, false ) );
	    else
    ...
    if ( (oper = keyword_lookup( fn_evals, buf )) < 0 )
    {
		bugf("Cmd_eval_obj: prog %d syntax error(5): '%s'",vnum,original);
		return false;
    }
    one_argument( line, buf );
    rval = atoi( buf );
	...
	const void *arg1, const void *arg2, CHAR_DATA *rch )
{
	...
	if ( (oper = keyword_lookup( fn_evals, buf )) < 0 )
	{
	    bugf("Cmd_eval_room: prog %d syntax error(2) '%s'",vnum,original);
	    return false;
	}
	one_argument( line, buf );
	lval = rval;
    ...
    if ( buf[0] != '$' || buf[1] == '\0' )
    {
		bugf("Cmd_eval_room: prog %d syntax error(3) '%s'",vnum,original);
        return false;
    }
    else
        code = buf[1];
    switch( code )
    {
        ...
        case 't':
            lval_char = vch; break;
        case 'r':
            lval_char = rch == NULL ? get_random_pc( NULL, NULL, room ) : rch ; break;
        case 'o':
            lval_obj = obj1; break;
        case 'p':
            lval_obj = obj2; break;
	...
	default:
	    bugf("Cmd_eval_room: prog %d syntax error(4) '%s'",vnum,original);
	    return false;
    }
    /*
     * From now on, we need an actor, so if none was found, bail out
	...
	*/
	...
	case CHK_ACT:
	    return( lval_char != NULL && lval_char->hasacts(buf) );
	case CHK_IMM:
	    return false; //( lval_char != NULL && lval_char->hasimms(buf) ); NASHNEESDTOFIXTHIS
	case CHK_OFF:
	    return( lval_char != NULL && lval_char->hasoffs(buf) );
	case CHK_CARRIES:
	    if ( is_number( buf ) )
    ...
    if ( (oper = keyword_lookup( fn_evals, buf )) < 0 )
    {
		bugf("Cmd_eval_room: prog %d syntax error(5): '%s'",vnum,original);
		return false;
    }
    one_argument( line, buf );
    rval = atoi( buf );
 ...
 * ------------------------------------------------------------------------
 */
void expand_arg_mob(char *buf,const char *format,CHAR_DATA *mob,CHAR_DATA *ch,const void *arg1,const void *arg2,CHAR_DATA *rch)
{
    ...
    while ( *str != '\0' )
    {
        ...
        switch ( *str )
        {
            ...
            case 'r':
				if ( rch == NULL ) 
					rch = get_random_pc(mob,NULL,NULL);
				i = someone;
			...
			case 'q':
				...
				break;
			case 'Q':
	    		i = (mob->mprog_target != NULL && can_see(mob,mob->mprog_target)) ? ( IS_NPC(mob->mprog_target) ? mob->mprog_target->short_descr : mob->mprog_target->name ) : someone;                         		break;
            case 'j':
				i = sex_table[SEX(mob)].eword;
				break;
            case 'e':
	    		i = (ch != NULL && can_see(mob,ch)) ? sex_table[SEX(ch)].eword : someone;
				break;
            case 'E':
	    		i = (vch != NULL && can_see(mob,vch)) ? sex_table[SEX(vch)].eword : someone;
				break;
            case 'J':
				i = (rch != NULL && can_see(mob,rch)) ? sex_table[SEX(rch)].eword : someone;
				break;
		    case 'X':
				i = (mob->mprog_target != NULL && can_see(mob,mob->mprog_target)) ? sex_table[SEX(mob->mprog_target)].eword : someone;
				break;
            case 'k':
				i = sex_table[SEX(mob)].mword;
				break;
            case 'm':
	    		i = (ch != NULL && can_see(mob,ch)) ? sex_table[SEX(ch)].mword : someone;
				break;
            case 'M':
		   		i = (vch != NULL && can_see(mob,vch)) ? sex_table[SEX(vch)].mword : someone;
				break;
            case 'K':
				if ( rch == NULL ) 
					rch = get_random_pc(mob,NULL,NULL);
				i = (rch != NULL && can_see(mob,rch)) ? sex_table[SEX(rch)].mword : someone;
				break;
            case 'Y':
	    		i = (mob->mprog_target != NULL && can_see(mob,mob->mprog_target)) ? sex_table[SEX(mob->mprog_target)].eword : someone;
				break;
            case 'l':
				i = sex_table[SEX(mob)].sword;
				break;
            case 's':
		    	i = (ch != NULL && can_see(mob,ch)) ? sex_table[SEX(ch)].sword : someones;
				break;
            case 'S':
	    		i = (vch != NULL && can_see(mob,vch)) ? sex_table[SEX(vch)].sword : someones;
				break;
            case 'L':
				if ( rch == NULL ) 
					rch = get_random_pc(mob,NULL,NULL);
				i = (rch != NULL && can_see(mob,rch)) ? sex_table[SEX(rch)].sword : someones;
				break;
            case 'Z':
		    	i = (mob->mprog_target != NULL && can_see(mob,mob->mprog_target)) ? sex_table[SEX(mob->mprog_target)].sword : someones;
				break;
			case 'o':
				i = something;
...
void expand_arg_other( char *buf, const char *format, OBJ_DATA *obj, ROOM_INDEX_DATA *room, CHAR_DATA *ch, const void *arg1, const void *arg2, CHAR_DATA *rch )
{
    const char *someone = "someone";
    const char *something = "something";
    const char *someones = "someone's";
    ...
    while ( *str != '\0' )
    {
        ...
        switch ( *str )
        {
            ...
            case 'r':
				if ( rch == NULL && obj ) 
					rch = get_random_pc(NULL,obj,NULL);
				else if ( rch == NULL && room )
					rch = get_random_pc(NULL,NULL,room);
				i = someone;
				if( rch != NULL )
				{
					one_argument( rch->name, fname );
			...
			case 'Q':
	    		i = (obj && obj->oprog_target != NULL)
					?
						...
						?
							obj->oprog_target->short_descr
							:
							obj->oprog_target->name)
					:
						(room && room->rprog_target != NULL)
						?
							( IS_NPC(room->rprog_target)
							?
								room->rprog_target->short_descr
								:
								room->rprog_target->name )
						: someone;
				break;
            case 'j':
				...
				break;
			case 'e':
	    		i = (ch != NULL) ? sex_table[SEX(ch)].eword : someone;
				break;
            case 'E':
	    		i = (vch != NULL) ? sex_table[SEX(vch)].eword : someone;
				break;
            case 'J':
				i = (rch != NULL) ? sex_table[SEX(rch)].eword : someone;
				break;
			case 'X':
				i = (obj && obj->oprog_target != NULL) ? sex_table[SEX(obj->oprog_target)].eword : (room && room->rprog_target != NULL) ? sex_table[SEX(room->rprog_target)].eword : someone;
				break;
            case 'k':
				...
				break;
            case 'm':
	    		i = (ch != NULL) ? sex_table[SEX(ch)].mword : someone;
				break;
            case 'M':
	    		i = (vch != NULL) ? sex_table[SEX(vch)].mword : someone;
				break;
            case 'K':
				if (obj && rch == NULL) 
					rch = get_random_pc(NULL,obj,NULL);
					...
					rch = get_random_pc(NULL,NULL,room);
				i = (rch != NULL) ? sex_table[SEX(rch)].mword : someone;
				break;
            case 'Y':
	    		i = (obj && obj->oprog_target != NULL) ? sex_table[SEX(obj->oprog_target)].mword : (room && room->rprog_target != NULL) ? sex_table[SEX(room->rprog_target)].mword : someone;
				break;
            case 'l':
				...
				break;
            case 's':
	    		i = (ch != NULL) ? sex_table[SEX(ch)].sword : someones;
				break;
            case 'S':
	    		i = (vch != NULL) ? sex_table[SEX(vch)].sword : someones;
				break;
            case 'L':
				if ( obj && rch == NULL ) 
					rch = get_random_pc(NULL,obj,NULL);
					...
					rch = get_random_pc(NULL,NULL,room);
				i = (rch != NULL) ? sex_table[SEX(rch)].sword : someones;
				break;
            case 'Z':
	    		i = (obj && obj->oprog_target != NULL) ? sex_table[SEX(obj->oprog_target)].sword : (room && room->rprog_target != NULL) ? sex_table[SEX(room->rprog_target)].mword : someones;
				break;
			case 'o':
				i = something;
				if ( obj1 != NULL )
...
void program_flow( sh_int pvnum, char *source, CHAR_DATA *mob, OBJ_DATA *obj, ROOM_INDEX_DATA *room,CHAR_DATA *ch, const void *arg1, const void *arg2, int numlines ){
    CHAR_DATA *rch = NULL;
    char *code, *line;
    char buf[MSL], control[MIL], data[MSL];
	...
	int count = 0;
	PROG_CODE *prog = get_prog_by_vnum(pvnum);
 ...
 * phrase.
 */
void p_act_trigger( char *argument, CHAR_DATA *mob, OBJ_DATA *obj, ROOM_INDEX_DATA *room, CHAR_DATA *ch, const void *arg1, const void *arg2, int type ){
    PROG_LIST *prg;
	SLEEP_DATA *test;
 ...
 */
bool p_percent_trigger(CHAR_DATA *mob,OBJ_DATA *obj,ROOM_INDEX_DATA *room,CHAR_DATA *ch,const void *arg1,const void *arg2,int type){
	PROG_LIST *prg;
	SLEEP_DATA *test;
...
bool p_exit_trigger( CHAR_DATA *ch, int dir, int type ){
    ...
    if ( type == PRG_MPROG )
    {
		...
		{    
			if ( IS_NPC(mob) && ( HAS_TRIGGER_MOB(mob,TRIG_EXIT) || HAS_TRIGGER_MOB(mob,TRIG_EXALL) ) )
			{
				for ( prg = mob->pIndexData->mprogs; prg; prg = prg->next )
				{
					/*
...
void p_greet_trigger( CHAR_DATA *ch, int type )
{
    ...
    if ( type == PRG_MPROG )
    {
		for ( mob = ch->in_room->people; mob != NULL; mob = mob->next_in_room )
		{    
			if ( IS_NPC(mob) && ( HAS_TRIGGER_MOB(mob,TRIG_GREET) || HAS_TRIGGER_MOB(mob,TRIG_GRALL) ) )
			{
				/*
	 			...
	 			* GrAll trigger
	 			*/
				if ( HAS_TRIGGER_MOB(mob,TRIG_GREET) && mob->position == mob->pIndexData->default_pos && can_see(mob,ch) )
			...
			if (IS_NPC(mob) && mob->isact(AT_GUARD) && !IS_NPC(ch))
			{
				if (ch->isplr(PL_KILLER))
				{
					sprintf(buf,"%s is a murderer! %s must be removed from the city!",ch->name, ch->sex == SEX_MALE ? "He" : ch->sex == SEX_FEMALE ? "She" : "It");
					do_function(mob, &do_yell, buf );
					do_function(mob, &do_kill, ch->name);
				}
				if (ch->isplr(PL_THIEF))
				{
					sprintf(buf,"THIEF! %s is violating our city!",ch->name);
					do_function(mob, &do_yell, buf );
					do_function(mob, &do_kill, ch->name);
				}
			}
