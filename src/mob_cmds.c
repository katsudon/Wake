...
void spell_null							( int,int,CHAR_DATA*,void*,int,int );
/*
 * MOB command table.
 */
...
char *prog_type_to_name( int type )
{
    ...
    case TRIG_INTERP:	return "INTERP";
...
void do_mob( CHAR_DATA *ch, char *argument )
{
     ...
     * Security check!
     */
    if ( ch->desc != NULL && get_trust(ch) < MAX_LEVEL-2 )
		return;
...
{
    char buf[MSL], command[MAX_INPUT_LENGTH];
    int cmd;
    ...
    bugf("Mob_interpret: invalid cmd from mob %d: '%s'",IS_NPC(ch) ? ch->pIndexData->vnum : 0,command);
	wiznet(buf,ch,NULL,WZ_PENALTIES,0,0);
}
...
void do_mpstat( CHAR_DATA *ch, char *argument )
{
    ...
    printf_to_char(ch,"Delay   %-6d [%s]\n\r",victim->mprog_delay,victim->mprog_target == NULL ? "No target" : victim->mprog_target->name);
...
void do_mpassist( CHAR_DATA *ch, char *argument )
{
    ...
    multi_hit( ch, victim->fighting, TYPE_UNDEFINED,false);
    return;
}
...
void do_mpmload( CHAR_DATA *ch, char *argument )
{
    ...
    {
		bugf("Mpmload: bad mob index (%d) from mob %d",vnum,IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
    }
...
void do_mppurge( CHAR_DATA *ch, char *argument )
{
	...
	for ( victim = ch->in_room->people; victim != NULL; victim = vnext )
	{
	    ...
	    if ( IS_NPC( victim ) && victim != ch && !victim->isact(AT_NOPURGE) )
	...
	{
	    obj_next = obj->next_content;
	    if (!IS_SET(obj->extra_flags,ITM_NOPURGE))
		extract_obj( obj );
	}
    ...
    if ( !IS_NPC( victim ) )
    {
		bug( "Mppurge - Purging a PC from vnum %d.", IS_NPC(ch) ? ch->pIndexData->vnum : 0 );
		return;
    }
    extract_char( victim, true );
    return;
...
void do_mptransfer(CHAR_DATA *ch,char *argument){
	...
	for(victim = char_list;victim;victim = victim->next)
		if(victim->iscomm(CM_DEBUG))
			printf_to_char(victim,"{B%s|%s|%s{x\n\r",argument,arg1,arg2);
	...
	if(!arg2[0])
		...
		if(!(location = find_location(ch,arg2))){
			bug("Mptransfer - No such location from vnum %d.",IS_NPC(ch) ? ch->pIndexData->vnum : 0);
			return;
		}
...
void do_stransfer(CHAR_DATA *ch,CHAR_DATA *victim,char *argument){
	ROOM_INDEX_DATA *location;
...
void do_mpgtransfer( CHAR_DATA *ch, char *argument ){
	...
	if (!(who = get_char_room(ch,NULL,arg1)))
		return;
...
/*
 * Lets the mobile transfer all chars following the victim.
 *
 * Syntax: mob atransfer [victim] [location]
 */
void recursive_follow_trans(CHAR_DATA *ch,CHAR_DATA *victim,CHAR_DATA *who,char *arg2){
	CHAR_DATA *pet,*pet_next;
	char buf[MSL];

	for(pet = victim->in_room->people;pet;pet = pet_next){
		pet_next = pet->next_in_room;
		if(pet->master == victim && pet != who && pet != victim){
			recursive_follow_trans(ch,pet,who,arg2);
			do_stransfer(ch,pet,arg2);
			continue;
		}
	}
}
...
void do_mpatransfer(CHAR_DATA *ch,char *argument){
	...
	recursive_follow_trans(ch,who,who,arg2);
	sprintf(buf,"%s %s",who->name,arg2);
	do_stransfer(ch,who,arg2);
}
...
void do_mpcast( CHAR_DATA *ch, char *argument )
{
    ...
    (*skill_table[sn].spell_fun)( sn, ch->level, ch, victim,skill_table[sn].target,1);//nash slvl
    return;
}
...
void do_mpdamage( CHAR_DATA *ch, char *argument){
	...
	if (!str_cmp(target,"all"))
		fAll = true;
	else if (!(victim = get_char_room(ch,NULL,target)))
		return;
	...
	if (!str_cmp(elem,"neutral") || !str_cmp(elem,"none"))
		element = DAM_OTHER;

	for (i = 0;dam_flags[i].name;i++){
		if (!str_cmp(dam_flags[i].name,elem)){
			element = dam_flags[i].bit;
			break;
		}
	}
	if (element == -1){
		bug("MpDamage - Shitty element mvnum %d!",IS_NPC(ch) ? ch->pIndexData->vnum : 0);
		return;
	}
	...
	if (fAll){
		for(victim = ch->in_room->people;victim;victim = victim_next){
			victim_next = victim->next_in_room;
			if (victim != ch)
    			damage(victim,victim,fKill ? number_range(low,high) : UMIN(victim->hit,number_range(low,high)),TYPE_UNDEFINED,element,false);
		}
	}
	else
		damage( victim, victim, fKill ? number_range(low,high) : UMIN(victim->hit,number_range(low,high)), TYPE_UNDEFINED, element, false );
...
void do_mpremember( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] != '\0' )
		...
		bug( "MpRemember: missing argument from vnum %d.", IS_NPC(ch) ? ch->pIndexData->vnum : 0 );
}
...
void do_mpcall( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
		bug( "MpCall: missing arguments from vnum %d.", IS_NPC(ch) ? ch->pIndexData->vnum : 0 );
		return;
    }
    if ( ( prg = get_prog_index( atoi(arg), PRG_MPROG ) ) == NULL )
    {
		bug( "MpCall: invalid prog from vnum %d.", IS_NPC(ch) ? ch->pIndexData->vnum : 0 );
		return;
    }
    vch = NULL;
    obj1 = obj2 = NULL;
...
void obj_interpret( OBJ_DATA *obj, char *argument )
{
    ...
    {
		if ( command[0] == obj_cmd_table[cmd].name[0] && !str_prefix(command,obj_cmd_table[cmd].name) )
		{
    ...
    bugf("Obj_interpret: invalid cmd from obj %d: '%s'",obj->pIndexData->vnum,command);
}
...
void do_opstat( CHAR_DATA *ch, char *argument )
{
    ...
    printf_to_char(ch,"Object #%-6d [%s]\n\r",obj->pIndexData->vnum,obj->short_descr);

    printf_to_char(ch,"Delay   %-6d [%s]\n\r",obj->oprog_delay,obj->oprog_target == NULL ? "No target" : obj->oprog_target->name);
    ...
    for ( i = 0, oprg = obj->pIndexData->oprogs; oprg != NULL; oprg = oprg->next )
		printf_to_char(ch,"[%2d] Trigger [%-8s] Program [%4d] Phrase [%s]\n\r",++i,prog_type_to_name(oprg->trig_type),oprg->vnum,oprg->trig_phrase);
    return;
...
void do_opzecho( OBJ_DATA *obj, char *argument )
{
    ...
    for ( d = descriptor_list; d; d = d->next )
    {
		if(d->connected == CON_PLAYING && d->character->in_room != NULL 
			&& ((obj->in_room && d->character->in_room->area == obj->in_room->area) || (obj->carried_by && d->character->in_room->area == obj->carried_by->in_room->area) ) )
 		{
			if ( IS_IMMORTAL(d->character) )
				send_to_char( "Obj echo> ", d->character );
			send_to_char( argument, d->character );
...
void do_opechoaround( OBJ_DATA *obj, char *argument )
{
    ...
    for ( ; vch; vch = vch->next_in_room )
    {
		if ( vch == victim )
			continue;
		act(argument,victim,obj,vch,TO_VICT);
    }
}
...
void do_opmload( OBJ_DATA *obj, char *argument )
{
    ...
    if ( (obj->in_room == NULL && (obj->carried_by == NULL || obj->carried_by->in_room == NULL)) || arg[0] == '\0' || !is_number(arg) )
		return;

    vnum = atoi(arg);
    if ( ( pMobIndex = get_mob_index( vnum ) ) == NULL )
    {
		bugf("Opmload: bad mob index (%d) from obj %d", vnum, obj->pIndexData->vnum);
		return;
    }
    victim = create_mobile( pMobIndex );
...
void do_oppurge( OBJ_DATA *obj, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
		...
		{
			vnext = victim->next_in_room;
			if ( IS_NPC(victim) && !victim->isact(AT_NOPURGE) )
				extract_char( victim, true );
		}
		...
		for ( ; vobj != NULL; vobj = obj_next )
		{
			...
			if (!IS_SET(vobj->extra_flags,ITM_NOPURGE) && vobj != obj)
			extract_obj( vobj );
		}
...
void do_opforce( OBJ_DATA *obj, char *argument )
{
    ...
    if ( !str_cmp( arg, "all" ) )
    {
		...
		for ( vch = char_list; vch != NULL; vch = vch_next )
		{
			...
			if ( ((obj->in_room && vch->in_room == obj->in_room) || (obj->carried_by && vch->in_room == obj->carried_by->in_room)) && get_trust(vch) < obj->level )
				interpret( vch, argument );
		}
    }
    else
...
void do_opvforce( OBJ_DATA *obj, char *argument )
{
    ...
    {
		victim_next = victim->next;
		if ( IS_NPC(victim) && victim->pIndexData->vnum == vnum && victim->fighting == NULL )
			interpret( victim, argument );
    }
    return;
}
    ...
    if ( fAll )
    {
		...
		for( ; victim; victim = victim_next )
		{
			...
			if ( obj->carried_by && victim != obj->carried_by )
    			damage( victim, victim, fKill ? number_range(low,high) : UMIN(victim->hit,number_range(low,high)),TYPE_UNDEFINED, DAM_NONE, false );
		}
    }
    else
    	damage( victim, victim, fKill ? number_range(low,high) : UMIN(victim->hit,number_range(low,high)), TYPE_UNDEFINED, DAM_NONE, false );
...
void do_opotransfer( OBJ_DATA *obj, char *argument )
{
    ...
    {
		bug( "OpOTransfer - No such location from vnum %d.", obj->pIndexData->vnum );
		return;
    }
 ...
 * Useful for removing e.g. quest objects from a character.
 *
 * Syntax: obj remove [victim] ['all']
 Nash: because the vnum part didn't work, the "all" is optional to remove just itself or all of it
 */
void do_opremove(OBJ_DATA *obj,char *argument){
	...
	if (!(victim = get_char_room(NULL,obj->in_room?obj->in_room:obj->carried_by->in_room, arg)))
		return;
...
void do_opattrib( OBJ_DATA *obj, char *argument )
{
    ...
    else if ( !isdigit( arg1[0] ) )
    {
		...
		if ( !is_number(arg1) )
		{
			bug( "OpAttrib: received non-number argument from vnum %d", obj->pIndexData->vnum );
			return;
		}
...
void room_interpret( ROOM_INDEX_DATA *room, char *argument )
{
    ...
    {
		if ( command[0] == room_cmd_table[cmd].name[0] && !str_prefix( command, room_cmd_table[cmd].name ) )
		{
    ...
    bugf("Room_interpret: invalid cmd from room %d: '%s'",room->vnum,command);
}
...
void do_rpstat( CHAR_DATA *ch, char *argument )
{
    ...
    printf_to_char(ch,"Room #%-6d [%s]\n\r",room->vnum,room->name);

    printf_to_char(ch,"Delay   %-6d [%s]\n\r",room->rprog_delay,room->rprog_target == NULL ? "No target" : room->rprog_target->name);
    ...
    for ( i = 0, rprg = room->rprogs; rprg != NULL; rprg = rprg->next )
		printf_to_char(ch,"[%2d] Trigger [%-8s] Program [%4d] Phrase [%s]\n\r",++i,prog_type_to_name(rprg->trig_type),rprg->vnum,rprg->trig_phrase);
...
void do_rpzecho( ROOM_INDEX_DATA *room, char *argument )
{
    ...
    for ( d = descriptor_list; d; d = d->next )
    {
		if ( d->connected == CON_PLAYING && d->character->in_room != NULL && d->character->in_room->area == room->area )
 		{
			if ( IS_IMMORTAL(d->character) )
				send_to_char( "Room echo> ", d->character );
			send_to_char( argument, d->character );
...
void do_rpasound( ROOM_INDEX_DATA *room, char *argument )
{
    ...
    for ( door = 0; door < 6; door++ )
    {
    	EXIT_DATA       *pexit;
      	...
      	if ( (pexit = room->exit[door]) != NULL && pexit->u1.to_room != NULL && pexit->u1.to_room != room && pexit->u1.to_room->people != NULL )
      	{
...
void do_rpmload( ROOM_INDEX_DATA *room, char *argument )
{
    ...
    if ( ( pMobIndex = get_mob_index( vnum ) ) == NULL )
    {
		bugf("Rpmload: bad mob index (%d) from room %d",vnum,room->vnum);
		return;
    }
    victim = create_mobile( pMobIndex );
